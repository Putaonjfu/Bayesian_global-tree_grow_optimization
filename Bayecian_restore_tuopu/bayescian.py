...
def bayesian_topology_reconstruction(skeleton_points, point_cloud, k, alpha, sigma, a, b, kappa, mu_theta):
    ...
    for u, v in candidate_graph.edges():  # 遍历每条边
        ...
        log_prior = -alpha * edge_length  # 先验概率，基于边长

        # Gamma 边长先验
        gamma_prior = (a - 1) * np.log(edge_length) - edge_length / b

        # 角度先验（冯·米塞斯分布）
        angle_diff = np.arccos(np.dot(p1, p2) / (np.linalg.norm(p1) * np.linalg.norm(p2)))  # 计算角度
        ...
        for pt in associated_points:  # 遍历与这条边匹配的那些最近的tls点
            ap = pt - p1
            ab = p2 - p1
            len_sq = np.dot(ab, ab)  # 计算边向量
            t = np.dot(ap, ab) / len_sq  # 计算投影系数 t
            if 0 <= t <= 1:  # 只保留投影在线段内部的点
                projection = p1 + t * ab
        ...
        # 计算总的贝叶斯权重
        weight = -(log_likelihood + gamma_prior + von_mises_prior)
...
if __name__ == '__main__':
    ...
    if skeleton_points is None or point_cloud is None:
        ...
        a = 1.2  # Gamma分布的形状参数
        b = 0.044  # Gamma分布的尺度参数
        kappa = 2.16  # 冯·米塞斯分布的集中度参数
        mu_theta = np.pi / 4  # 角度先验期望角度（45度）
